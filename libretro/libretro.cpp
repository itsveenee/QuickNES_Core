...
/* SNESTICLE_HARD_RESET
 * libretro's retro_reset() intentionally maps to QuickNES's partial/soft
 * reset. SNESticle has separate Reset() and SoftReset() commands, so expose
 * the full-reset path without changing the libretro API behaviour.
 */
extern "C" void quicknes_snesticle_hard_reset(void)
{
   if (emu)
      emu->reset( true, false );
}
...
void retro_run(void)
{
   ...
   if (!audioDisabledForThisFrame)
   {
	   ...
	   static int16_t out_samples[8192];
